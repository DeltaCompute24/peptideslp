...
import { protocolDictionaryPT as protocolDictionary, protocolCategoriesPT as protocolCategories, reconstitutionGuidePT as reconstitutionGuide, peptideGlossaryPT as peptideGlossary } from '../data/protocolDictionaryPT';
...
export default function EBookPT() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const doc = generateEbookPDF(protocolDictionary, protocolCategories, reconstitutionGuide, peptideGlossary, 'pt');
    ...
    <nav style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(13,148,136,0.1)', padding: '0.75rem 2rem' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
          {[
            { id: 'toc', label: 'Sumario' },
            { id: 'protocols', label: 'Protocolos' },
            { id: 'reconstitution', label: 'Como Usar' },
          ...
          <Link href="/wizard" style={{ padding: '6px 16px', borderRadius: '20px', border: '1px solid #0d9488', background: 'rgba(13,148,136,0.08)', color: '#0d9488', fontFamily: '"DM Sans", sans-serif', fontSize: '0.8rem', fontWeight: 600, textDecoration: 'none', transition: 'all 0.2s' }}>
            Protocol Wizard
          </Link>
  ...
  const renderTOC = () => (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 1.5rem' }}>
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#0d9488', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>E-Book BioSync Peptideos</div>
        ...
        <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '1.05rem', color: '#64748b', maxWidth: '600px', margin: '0 auto', lineHeight: 1.7 }}>
          Protocolos baseados em evidencias, guias de dosagem e dados cientificos para 27 peptideos em 8 categorias terapeuticas. Apenas para fins educacionais e de pesquisa.
        </p>
        <button onClick={handleDownloadPDF} disabled={isGeneratingPDF}
          style={{
      ...
      <div style={{ display: 'grid', gap: '1rem' }}>
          ...
          <button key={cat.id}
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                <span style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.7rem', fontWeight: 700, color: '#0d9488', opacity: 0.6 }}>CAPITULO {i + 1}</span>
              </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '0.5rem' }}>
          <button onClick={() => { setActiveSection('reconstitution'); setShowGlossary(false); }} className="card-hover"
            ...
            <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#0d9488', letterSpacing: '0.1em', marginBottom: '0.35rem' }}>APENDICE A</div>
            <h3 style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '1rem', fontWeight: 700, color: '#0c1929', marginBottom: '0.25rem' }}>Guia de Reconstituicao</h3>
            <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.78rem', color: '#64748b', margin: 0 }}>Passo a passo de mistura, formulas de dosagem, conversao de seringas, tecnica de injecao</p>
          </button>
          <button onClick={() => { setActiveSection('glossary'); setShowGlossary(true); }} className="card-hover"
            style={{ padding: '1.25rem', borderRadius: '16px', border: '1px solid rgba(13,148,136,0.12)', background: 'linear-gradient(135deg, rgba(13,148,136,0.03), rgba(13,148,136,0.08))', cursor: 'pointer', textAlign: 'left' }}>
            <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#0d9488', letterSpacing: '0.1em', marginBottom: '0.35rem' }}>APENDICE B</div>
            <h3 style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '1rem', fontWeight: 700, color: '#0c1929', marginBottom: '0.25rem' }}>Glossario</h3>
            <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.78rem', color: '#64748b', margin: 0 }}>26 termos-chave — de Aminoacido a Titulacao</p>
          </button>
        </div>
      </div>
    </div>
  ...
  const renderProtocolDetail = (protocol) => (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      <button onClick={() => setSelectedProtocol(null)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'none', border: 'none', color: '#0d9488', cursor: 'pointer', fontFamily: '"DM Sans", sans-serif', fontWeight: 600, fontSize: '0.85rem', marginBottom: '1.5rem', padding: 0 }}>
        ...
        Voltar para {activeCategory ? protocolCategories.find(c => c.id === activeCategory)?.name : 'resultados'}
      </button>
      ...
      {/* Mechanism */}
      {protocol.mechanism && (
        <Section title="Mecanismo de Acao">
      ...
      {/* Protocol */}
      {protocol.protocol && (
        <Section title="Protocolo de Dosagem">
      ...
      {protocol.researchHighlights && (
        <Section title="Destaques da Pesquisa">
      ...
      {protocol.contraindications && (
        <Section title="Contraindicacoes" accent="#dc2626">
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
      ...
      {protocol.sideEffects && (
        <Section title="Efeitos Colaterais Potenciais">
      ...
      <div style={{ marginTop: '2rem', padding: '1rem', borderRadius: '12px', background: '#fefce8', border: '1px solid #fde047' }}>
        <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.75rem', color: '#854d0e', lineHeight: 1.6, margin: 0 }}>
          <strong>Aviso:</strong> Esta informacao e apenas para fins educacionais e de pesquisa. Nao constitui aconselhamento medico. Sempre consulte um profissional de saude qualificado antes de iniciar qualquer terapia com peptideos. As respostas individuais variam. Muitos peptideos discutidos aqui nao sao aprovados pela FDA para os usos descritos.
        </p>
      </div>
    </div>
  );
  ...
  const renderProtocolsList = () => (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <input
          type="text"
          placeholder="Buscar peptideos por nome, categoria ou classe..."
          value={searchQuery}
      ...
      {searchQuery && (
        <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.85rem', color: '#64748b', marginBottom: '1rem' }}>
          {filteredProtocols.length} resultado{filteredProtocols.length !== 1 ? 's' : ''} para "{searchQuery}"
        </p>
      )}

      {!activeCategory && !searchQuery && (
        <div style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>
          <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.95rem' }}>Selecione uma categoria acima ou pesquise para navegar pelos protocolos</p>
        </div>
      )}
  ...
  const renderReconstitution = () => (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      <div style={{ marginBottom: '2.5rem' }}>
        <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#0d9488', letterSpacing: '0.12em', marginBottom: '0.5rem' }}>APENDICE A</div>
        <h1 style={{ fontFamily: '"Playfair Display", serif', fontSize: 'clamp(1.8rem, 4vw, 2.5rem)', fontWeight: 700, color: '#0c1929', marginBottom: '0.5rem' }}>Guia de Reconstituicao e Administracao</h1>
        <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.9rem', color: '#64748b', lineHeight: 1.6 }}>Como reconstituir, dosar, injetar e armazenar peptideos corretamente.</p>
      </div>
      ...
      {/* Steps */}
      <Section title="Etapas de Reconstituicao">
        <div style={{ display: 'grid', gap: '0.75rem' }}>
      ...
      {/* Dosing Formula */}
      <Section title="Calculo de Dosagem">
        <div style={{ padding: '1.25rem', borderRadius: '14px', background: 'linear-gradient(135deg, #0c1929, #1e3a5f)', color: 'white', marginBottom: '1rem' }}>
          ...
          <div style={{ fontFamily: 'monospace', fontSize: '0.85rem', lineHeight: 2 }}>
            <div>Concentracao = Quantidade do Peptideo (mcg) / Agua BAC (mL)</div>
            <div>Vol. Injecao = Dose Alvo (mcg) / Concentracao (mcg/mL)</div>
          </div>
        </div>
        <div style={{ padding: '1rem', borderRadius: '12px', background: '#f0fdf4', border: '1px solid #86efac' }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#166534', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>EXEMPLO</div>
          <div style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.85rem', color: '#166534', lineHeight: 1.8 }}>
            <div>5mg peptideo + 2 mL agua BAC = <strong>2.500 mcg/mL</strong></div>
            <div>Dose alvo: 500 mcg / 2.500 mcg/mL = <strong>0,2 mL (20 unidades)</strong></div>
          </div>
        </div>
      </Section>

      {/* Syringe Conversion */}
      <Section title="Conversao de Seringa (U-100)">
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: '"DM Sans", sans-serif', fontSize: '0.85rem' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #0d9488' }}>
                <th style={{ padding: '0.6rem', textAlign: 'left', color: '#0f766e', fontWeight: 700 }}>Unidades</th>
                <th style={{ padding: '0.6rem', textAlign: 'left', color: '#0f766e', fontWeight: 700 }}>mL</th>
                <th style={{ padding: '0.6rem', textAlign: 'left', color: '#0f766e', fontWeight: 700 }}>Se 2.500 mcg/mL</th>
              </tr>
            </thead>
            <tbody>
              {reconstitutionGuide.syringeConversion.map((row, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '0.5rem 0.6rem', fontWeight: 600 }}>{row.units} unid.</td>
      ...
      {/* Injection Sites */}
      <Section title="Locais de Injecao e Tecnica">
        ...
        <div style={{ padding: '1rem', borderRadius: '12px', background: '#f8fafc', border: '1px solid #e2e8f0' }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#64748b', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>ETAPAS DA INJECAO</div>
      ...
      {/* Storage */}
      <Section title="Diretrizes de Armazenamento">
  ...
  const renderGlossary = () => (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      <div style={{ marginBottom: '2rem' }}>
        <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#0d9488', letterSpacing: '0.12em', marginBottom: '0.5rem' }}>APENDICE B</div>
        <h1 style={{ fontFamily: '"Playfair Display", serif', fontSize: 'clamp(1.8rem, 4vw, 2.5rem)', fontWeight: 700, color: '#0c1929', marginBottom: '0.5rem' }}>Glossario</h1>
        <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '0.9rem', color: '#64748b' }}>Termos-chave utilizados neste dicionario de protocolos.</p>
      </div>
  ...
  return (
    <>
      <Head>
        <title>Dicionario de Protocolos de Peptideos | BioSync E-Book</title>
        <meta name="description" content="Dicionario completo de protocolos de peptideos baseado em evidencias com guias de dosagem, instrucoes de reconstituicao e dados cientificos para 27 peptideos." />
      ...
      <div style={{ minHeight: '100vh', background: '#fafbfc', fontFamily: '"DM Sans", sans-serif' }}>
        ...
        <footer style={{ borderTop: '1px solid rgba(13,148,136,0.1)', padding: '2rem 1.5rem', textAlign: 'center', marginTop: '2rem' }}>
          <p style={{ fontSize: '0.72rem', color: '#94a3b8', lineHeight: 1.8, maxWidth: '600px', margin: '0 auto' }}>
            Este e-book e apenas para fins educacionais e de pesquisa. Nao constitui aconselhamento medico. Consulte um profissional de saude qualificado antes de iniciar qualquer terapia com peptideos. A BioSync Peptides nao afirma diagnosticar, tratar, curar ou prevenir qualquer doenca.
          </p>
